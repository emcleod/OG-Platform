 ...
 * @author emcleod
 */
public class ExampleCallPutVolatilitySurfaceConfigPopulator {
  /** The logger */
  private static final Logger s_logger = LoggerFactory.getLogger(ExampleCallPutVolatilitySurfaceConfigPopulator.class);
  ...
  public static ConfigMaster populateSurfaceConfigMaster(final ConfigMaster configMaster, final Map<String, String> configs) throws IOException {
    ...
    try (InputStream resource = SimulatedHistoricalDataGenerator.class.getResourceAsStream("historical-data.csv")) {
      if (resource == null) {
        s_logger.error("Could not get file called historical-data.csv");
        return configMaster;
      }
      try (final CSVReader reader = new CSVReader(new BufferedReader(new InputStreamReader(resource)))) {
        ...
        if (!matchedTickers.containsAll(configs.keySet())) {
          s_logger.error("Could not get all configurations; missing {}", Sets.difference(configs.keySet(), matchedTickers));
        }
        return configMaster;
      }
    }
